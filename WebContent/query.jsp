	...
	$(function() {
        ...
        $("#goto").click(function () {
            ...
            if(gotoPage ==""||gotoPage<=0||gotoPage>maxPage){
                ...
                location.href="${pageContext.request.contextPath }/manager/homeCostServlet?action=query&keyword="+keyword+"&currentPage="+gotoPage+"&pageSize="+pageSize;
            }
        });
      ...
      //页数尺寸改变
        $("#pagesize").change(function(){
        	...
        	location.href="${pageContext.request.contextPath }/manager/homeCostServlet?action=query&keyword="+keyword+"&currentPage=1&pageSize="+pageSize;
		});
...
<%
	...
	System.out.println("cur page:query");
	...
	<div id="main">
		...
		<c:when test="${requestScope.pageBean.getList().size()>0}">
			<table style="margin-top: 30px">
				<tr>
					<td class="costname" style="width: 150px">消费名称</td>
					<td>消费金额</td>
				...
				<c:forEach items="${requestScope.pageBean.getList()}" var="item">
					<tr>
						...
						<td>${item.money}</td>
						<td>${item.date}</td>
						<td><a href="${pageContext.request.contextPath }/manager/homeCostServlet?action=showDetails&id=${item.id}">
						详情信息</a></td>
						<td><a href="${pageContext.request.contextPath }/manager/homeCostServlet?action=getHomeCostById&id=${item.id}">
						修改</a></td>
						<td><a class="deleteClass"href="${pageContext.request.contextPath }/manager/homeCostServlet?action=delete&id=${item.id}">
						删除</a></td>
					</tr>
				</c:forEach>
				<tr>
					...
					<td></td>
					<td colspan="3">
						<a href="${pageContext.request.contextPath }/cost_edit.jsp">新增</a>
					</td>
				</tr>
				<tr>
					<td colspan="6">
						<div style="line-height: 20px; height: 20px;">
							...
							<c:if test="${pageBean.currentPage!=1}">
			    				[<a href="${pageContext.request.contextPath }/manager/homeCostServlet?action=query&keyword=<%=request.getParameter("keyword")%>&currentPage=${pageBean.currentPage-1}&pageSize=${pageBean.pageSize}">
			    					前一页
								</a>]
							</c:if>
